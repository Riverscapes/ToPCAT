...
int export_stats(const _TCHAR* pcmmf_filename, const uint64 min_npoints_per_cell)
{
	...
	ofstream
		...
		outstat_underpopulated((filename+_T("_underpopulated")+_T("_stats.txt")).c_str());
	...
	outstat << "x,y,zmean,zmax,zmin,range,stdev,stdev_detrended,sk,sk_detr,ku,ku_detr,zmean_detrended,n"<< endl;
	outstat_underpopulated << "x,y,zmean,zmax,zmin,range,stdev,stdev_detrended,sk,sk_detr,ku,ku_detr,zmean_detrended,n"<< endl;
